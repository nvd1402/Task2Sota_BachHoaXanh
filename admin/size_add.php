...
$pageTitle = "Thêm kích thước - Admin Dashboard";
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $conn = connectDB();
        $check = $conn->prepare("SELECT id FROM sizes WHERE slug = ?");
        ...
        $stmt = $conn->prepare("INSERT INTO sizes (name, slug, description, status) VALUES (?, ?, ?, ?)");
        ...
        if ($stmt->execute()) {
            ...
            $errors[] = "Lỗi khi thêm kích thước: " . $stmt->error;
            $stmt->close();
            closeDB($conn);
        }
    }
                ...
                <div class="row mt-4">
                  <div class="col-12">
                    <button type="submit" class="btn" style="background-color:#000;border-color:#000;color:#fff;">
                      <i class="fas fa-save me-2"></i>Lưu kích thước
                    </button>
