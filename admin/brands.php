...
$pageTitle = "Quản lý Thương hiệu - Admin Dashboard";
...
$countStmt->execute();
$totalBrands = $countStmt->get_result()->fetch_assoc()['total'] ?? 0;
$countStmt->close();

$totalPages = max(1, ceil($totalBrands / $perPage));

// Danh sách
$sql = "SELECT * FROM brands $where ORDER BY created_at DESC LIMIT ? OFFSET ?";
    ...
    <div class="container-fluid py-4">
      <div class="row mb-4">
        <div class="col-lg-6 col-md-6 col-12">
          ...
          <p class="text-sm text-muted mb-0">Tổng cộng: <?= number_format($totalBrands) ?> thương hiệu</p>
        </div>
        <div class="col-lg-6 col-md-6 col-12 text-end">
          <a href="brand_add.php" class="btn btn-sm mb-0" style="background-color:#000;border-color:#000;color:#fff;">
      ...
      <div class="row mb-4">
        <div class="col-12">
          <div class="card">
            <div class="card-body">
              <form method="GET" action="brands.php" class="row g-3">
                <div class="col-md-10">
            ...
            <div class="card-body px-0 pb-2">
              <div class="table-responsive">
                <table class="table align-items-center mb-0">
                  <thead>
                    <tr>
                      <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Tên thương hiệu</th>
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>
                        <h6 class="mb-0 text-sm"><?= htmlspecialchars($brand['name']) ?></h6>
                      </td>
                      <td>
                        <p class="text-xs font-weight-bold mb-0"><?= htmlspecialchars($brand['slug']) ?></p>
                      </td>
                      <td>
                        <p class="text-xs mb-0 text-secondary">
                          <?= htmlspecialchars(mb_substr($brand['description'] ?? '', 0, 60)) ?><?= strlen($brand['description'] ?? '') > 60 ? '...' : '' ?>
                        </p>
                      </td>
                      <td class="align-middle">
                        <span class="text-xs font-weight-bold <?= $brand['status'] === 'active' ? 'text-success' : 'text-secondary' ?>">
                          <?= $brand['status'] === 'active' ? 'Hoạt động' : 'Ngừng dùng' ?>
                        </span>
                      </td>
                      <td class="align-middle text-center">
                        ...
                        <a href="brand_delete.php?id=<?= $brand['id'] ?>"
                           class="text-xs font-weight-bold"
                           style="color:#e53935; text-decoration:none;"
                           onclick="return confirm('Xóa thương hiệu này? Các sản phẩm đang dùng thương hiệu này sẽ không bị xóa.')">
                          Xóa
                        </a>
                      </td>
                    </tr>
