...
$pageTitle = "Sửa thương hiệu - Admin Dashboard";
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $check = $conn->prepare("SELECT id FROM brands WHERE slug = ? AND id != ?");
        ...
        $upd = $conn->prepare("UPDATE brands SET name = ?, slug = ?, description = ?, status = ? WHERE id = ?");
                  ...
                  <div class="col-md-4">
                    <div class="mb-3">
                      ...
                      <select name="status" class="form-control">
                        ...
                        <option value="inactive" <?= $brand['status'] === 'inactive' ? 'selected' : '' ?>>Ngừng dùng</option>
                      </select>
                    </div>
                  </div>
                </div>
